import { about_section_data } from "@/data/AboutUsData";
import Image from "next/image";
import Link from "next/link";

const About = ({
  should_display_about_btn = true,
}: {
  should_display_about_btn?: boolean;
}) => {
  return (
    <div className="td-about-area pt-140 pb-110">
      <div className="container">
        <div className="row align-items-center">
          <div
            className="col-xl-6 col-lg-9 wow fadeInLeft"
            data-wow-duration="1.5s"
            data-wow-delay="0.3s"
          >
            <div className="td-about-3-thumb mr-60 mb-30">
              <Image
                className="w-100"
                src={about_section_data.about_thumb}
                alt=""
              />
            </div>
          </div>
          <div
            className="col-xl-6 col-lg-9 wow fadeInRight"
            data-wow-duration="1.5s"
            data-wow-delay="0.3s"
          >
            <div className="td-about-3-content-wrap mb-15">
              <div className="td-about-2-content">
                <span className="td-section-subtitle td-section-subtitle-2 td-section-subtitle-3 mb-15">
                  {about_section_data.title}
                </span>
                <h2 className="td-section-title mb-15">
                  {about_section_data.sub_title}
                </h2>
                <p className="para para-2 mb-30">
                  {about_section_data.presentation1}
                </p>
                <div className="td-about-3-clients d-flex flex-wrap">
                  <div className="items mb-15">
                    <span className="mb-10 icon">
                      <i className="flaticon-meeting"></i>
                    </span>
                    <h5 className="title" style={{ maxWidth: "200px" }}>
                      {about_section_data.meetup_people}
                    </h5>
                  </div>
                  <div className="td-border d-none d-sm-block"></div>
                  <div className="items mb-15">
                    <span className="mb-10 icon">
                      <i className="flaticon-handshake"></i>
                    </span>
                    <h5 className="title" style={{ maxWidth: "200px" }}>
                      {about_section_data.connect_with}
                    </h5>
                  </div>
                </div>
                <p className="para mb-40">{about_section_data.presentation2}</p>
              </div>
              <div className="td-about-3-btn d-flex align-items-center">
                {should_display_about_btn && (
                  <Link
                    className="td-btn td-btn-3-squre mb-15 td-left-right text3 mr-25"
                    href={about_section_data.about_vukafrik_href}
                  >
                    <span className="mr10 td-text d-inline-block mr-5">
                      {about_section_data.about_vukafrik_title}
                    </span>
                    <span className="td-arrow-angle">
                      <svg
                        className="td-arrow-svg-top-right"
                        width="13"
                        height="14"
                        viewBox="0 0 13 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M0.943836 13.5C0.685616 13.5 0.45411 13.4021 0.276027 13.224C0.0979452 13.0459 0 12.8055 0 12.5562C0 12.3068 0.0979452 12.0664 0.276027 11.8884L9.76781 2.38767H2.02123C1.49589 2.38767 1.0774 1.96027 1.0774 1.44384C1.0774 0.927397 1.50479 0.5 2.03014 0.5H12.0562C12.1274 0.5 12.1986 0.508904 12.2788 0.526712L12.4034 0.562329L12.537 0.633562C12.5637 0.65137 12.5993 0.678082 12.626 0.69589C12.6973 0.749315 12.7507 0.80274 12.7952 0.856164C12.8219 0.891781 12.8575 0.927397 12.8842 0.989726L12.9555 1.1411L12.9822 1.22123C13 1.29247 13.0089 1.3726 13.0089 1.44384V11.4699C13.0089 11.9952 12.5815 12.4137 12.0651 12.4137C11.5486 12.4137 11.1212 11.9863 11.1212 11.4699V3.72329L1.62055 13.224C1.44247 13.4021 1.20205 13.5 0.943836 13.5Z"
                          fill="white"
                        />
                        <path
                          d="M0.943836 13.5C0.685616 13.5 0.45411 13.4021 0.276027 13.224C0.0979452 13.0459 0 12.8055 0 12.5562C0 12.3068 0.0979452 12.0664 0.276027 11.8884L9.76781 2.38767H2.02123C1.49589 2.38767 1.0774 1.96027 1.0774 1.44384C1.0774 0.927397 1.50479 0.5 2.03014 0.5H12.0562C12.1274 0.5 12.1986 0.508904 12.2788 0.526712L12.4034 0.562329L12.537 0.633562C12.5637 0.65137 12.5993 0.678082 12.626 0.69589C12.6973 0.749315 12.7507 0.80274 12.7952 0.856164C12.8219 0.891781 12.8575 0.927397 12.8842 0.989726L12.9555 1.1411L12.9822 1.22123C13 1.29247 13.0089 1.3726 13.0089 1.44384V11.4699C13.0089 11.9952 12.5815 12.4137 12.0651 12.4137C11.5486 12.4137 11.1212 11.9863 11.1212 11.4699V3.72329L1.62055 13.224C1.44247 13.4021 1.20205 13.5 0.943836 13.5Z"
                          fill="white"
                        />
                      </svg>
                    </span>
                  </Link>
                )}
                <div className="td-about-3-number mb-15 d-flex align-items-center">
                  <span className="call-icon mr-10">
                    <i className="flaticon-phone"></i>
                  </span>
                  <div>
                    <span className="subtitle">
                      {about_section_data.call_us_title}:
                    </span>
                    <h5 className="td-number">
                      <Link href={`tel:${about_section_data.call_us_phone}`}>
                        {about_section_data.call_us_phone}
                      </Link>
                    </h5>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default About;
