...
const About = () => {
   return (
      <div id={vukafrikAbout.id} className="td-about-area pt-140 pb-110">
               ...
               <div className="col-xl-6 col-lg-9 wow fadeInRight" data-wow-duration="1.5s" data-wow-delay="0.3s">
                  <div className="td-about-3-content-wrap mb-15">
                     <div className="td-about-2-content">
                        <span className="td-section-subtitle td-section-subtitle-2 td-section-subtitle-3 mb-15">{vukafrikAbout.eyebrow}</span>
                        <h2 className="td-section-title mb-15">{vukafrikAbout.title}</h2>
                        <p className="para para-2 mb-30">{vukafrikAbout.intro}</p>
                        <div className="td-about-3-clients d-flex flex-wrap">
                           {vukafrikAbout.highlights.map((item, index) => (
                              <div key={item.label} className="d-flex flex-wrap align-items-center">
                                 <div className="items mb-15">
                                    <span className="mb-10 icon">
                                       <i className={item.icon}></i>
                                    </span>
                                    <h5 className="title">{item.label}<br />{item.description}</h5>
                                 </div>
                                 {index === 0 ? <div className="td-border d-none d-sm-block"></div> : null}
                              </div>
                           ))}
                        </div>
                        <p className="para mb-40">{vukafrikAbout.body}</p>
                     </div>
                     <div className="td-about-3-btn d-flex align-items-center">
                        <Link className="td-btn td-btn-3-squre mb-15 td-left-right text3 mr-25" href={vukafrikEvent.ctaLink}>
                           <span className="mr10 td-text d-inline-block mr-5">{vukafrikEvent.ctaLabel}</span>
                        ...
                        <div className="td-about-3-number mb-15 d-flex align-items-center">
                           <span className="call-icon mr-10">
                              <i className="flaticon-teamwork-2"></i>
                           </span>
                           <div>
                              <span className="subtitle">{vukafrikAbout.supportingLabel}:</span>
                              <h5 className="td-number"><Link href="/contact">{vukafrikAbout.supportingValue}</Link></h5>
                           </div>
                        </div>
                     </div>
                  </div>
