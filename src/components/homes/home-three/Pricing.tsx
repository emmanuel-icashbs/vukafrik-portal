...
import { exhibition_stands } from "@/data/ExhibitionData";
          ...
          <div className="col-xl-6 col-lg-8">
            ...
            >
              <span className="td-section-subtitle td-section-subtitle-2 td-section-subtitle-3  mb-30">
                {exhibition_stands.sub_title}
              </span>
              <h2 className="td-section-title mb-10">
                {exhibition_stands.title}
              </h2>
              <p className="td-section-text">{exhibition_stands.description}</p>
            </div>
          </div>
        </div>
        <div className="row gx-30">
          {exhibition_stands.data.map((item) => (
            <div key={item.id} className="col-xl-4 col-lg-6 col-md-6 mb-30">
              <div
                className="td-pricing-2-item td-pricing-3-item wow fadeInLeft"
                data-wow-duration="1.5s"
                data-wow-delay="0.3s"
              >
                <div className="td-pricing-2-price text-center mb-10">
                  <span className="subtitle mb-25">{item.title}</span>
                  <span className="price">{item.price}.00</span>
                </div>
                <div className="td-pricing-2-feature mb-45">
                  <ul>
                    {item.advantages.map((list, i) => (
                      <li key={i}>
                        <span>
                          <svg
                            width="22"
                ...
                <Link
                  className="td-btn td-btn-3-squre td-left-right text3"
                  href={cta_data.become_exhibitor.href}
                >
                  <span className="mr10 td-text d-inline-block mr-5">
                    {exhibition_stands.submit_button}
                  </span>
