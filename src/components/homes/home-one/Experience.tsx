...
import { vukafrikCta } from "@/data/vukafrik-content";

const Experience = () => {
  useJarallax(0.2);

   return (
      <div className="td-banner-area td-banner-spacing bg-position jarallax" style={{ backgroundImage: `url(${vukafrikCta.background.src})` }}>
         <div className="container">
            <div className="row">
               <div className="col-xl-8 col-lg-10">
                  <div className="td-banner-title-wrap">
                     <span className="td-section-subtitle mb-20 wow fadeInLeft" data-wow-duration=".9s" data-wow-delay=".2s">{vukafrikCta.eyebrow}</span>
                     <h2 className="td-section-title mb-15 wow fadeInLeft" data-wow-duration=".9s" data-wow-delay=".4s">{vukafrikCta.title}</h2>
                     <p className="mb-35 wow fadeInLeft" data-wow-duration=".9s" data-wow-delay=".6s">{vukafrikCta.description}</p>
                     <div className="d-flex flex-wrap gap-3">
                        {vukafrikCta.actions.map((action, index) => (
                           <Link key={action.label} className="td-btn td-left-right wow fadeInLeft" data-wow-duration=".9s" data-wow-delay={index === 0 ? ".8s" : "1s"} href={action.link}>
